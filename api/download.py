...
from ..file.cleanup import _is_safe_cleanup_target, cleanup_temp_file
...
async def get_file_url(bot: Any, file_seg: Any) -> tuple[str, str] | None:
    """
    ...
    try:
        ...
        if not file_url:
            ...
            if file_field and (
                file_field.startswith("http://") or file_field.startswith("https://")
            ):
                file_url = file_field
            ...
            # 方法2: 尝试使用 file_id 通过 Bot API 获取文件信息
                ...
                try:
                    ...
                    if http_url:
                        ...
                        if local_path is not None and _is_safe_cleanup_target(
                            local_path
                        ):
                            asyncio.create_task(cleanup_temp_file(local_path))
                    elif local_file_str:
    ...
    try:
        ...
        if is_local_path and local_file_path:
            ...
            if max_file_size is not None and file_size > max_file_size:
                raise RuntimeError(
                    f"文件过大（{file_size / 1024 / 1024:.2f}MB）超过限制（{config.max_file_size_mb}MB）"
                )
                ...
                async with session.get(url) as resp:
                    if resp.status == 200:
                        ...
                        if content_length:
                            try:
                                ...
                                if max_file_size is not None and size > max_file_size:
                                    raise RuntimeError(
                                        f"文件过大（{size / 1024 / 1024:.2f}MB）超过限制（{config.max_file_size_mb}MB）"
                                    )
                            except ValueError:
                                pass
                        ...
                        if max_file_size is not None and len(content) > max_file_size:
                            logger.warning(
                                f"下载中发现文件超过限制（{config.max_file_size_mb}MB），中止下载"
                            )
                        ...
                        raise RuntimeError(f"下载失败，状态码：{resp.status}")
    except Exception as e:
        raise RuntimeError(f"下载异常：{e}") from e
